...
-- Update the trigger function to also trigger email send via pg_net
...
BEGIN
  ...
  IF _participant.receives_notifications THEN
    ...
    -- Use direct HTTP call via pg_net if available, otherwise skip email
    BEGIN
      PERFORM net.http_post(
        url := COALESCE(_supabase_url, 'https://nmqycanqumelmfpdmkpr.supabase.co') || '/functions/v1/order-message-email-send',
    ...
    EXCEPTION WHEN OTHERS THEN
      -- pg_net may not be available, email will not be sent
