  ...
  const addItemsBulk = useCallback(
    async (rows: Array<Partial<MaterialItemRow> & { description: string }>) => {
      ...
      const payload = rows.map((r, idx) => ({
        ...
        provided_by: r.provided_by ?? null,
        ...
        ai_source_type: r.ai_source_type ?? null,
        ai_source_file: r.ai_source_file ?? null,
        ai_source_page: r.ai_source_page ?? null,
        ai_source_label: r.ai_source_label ?? null,
        ai_component_reference: r.ai_component_reference ?? null,
