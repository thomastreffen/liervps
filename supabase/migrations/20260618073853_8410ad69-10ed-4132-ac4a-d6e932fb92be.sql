...
BEGIN
  ...
  LOOP
    -- Hard reset per-iteration state so values cannot leak across dates.
    _date := NULL;
    _start := NULL;
    _end := NULL;
    _per_req := NULL;
    ...
    BEGIN
      ...
      IF _end <= _start THEN
        _results := _results || jsonb_build_object(
          ...
          'error', format('invalid time range: end (%s) must be after start (%s) — caller must bump end_date for overnight ranges', _end, _start),
          'sqlstate', '22023'
        );
        CONTINUE;
      END IF;
